...
**Previous release:** [v0.1.0-alpha.10](https://github.com/kaywhy331/Video/releases/tag/v0.1.0-alpha.10) unsigned Windows prerelease
...
`production_ready` therefore remains `false`. See [Implementation Coverage](docs/IMPLEMENTATION-COVERAGE.md), [Production Hardening](docs/PRODUCTION-HARDENING.md), the historical [alpha.7 Validation Report](VALIDATION_REPORT.md), and the current release's machine-readable [alpha.10 release-evidence index](docs/release-evidence/v0.1.0-alpha.10.json). Current validation status and acceptance receipts are generated artifacts: run clean release validation or download the exact-workflow-SHA CI artifact rather than treating a tracked root receipt as current evidence.

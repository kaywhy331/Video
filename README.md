...
- SQLite/WAL/FTS5 data store with 19 atomic forward migrations, nested savepoints, composite catalog-search indexes, immutable Guided-input provenance, deferred lifecycle intent at active-job checkpoints, and source/package migration-parity preflight
...
2. In Settings, configure storage, backup policy, narrator and pronunciation dictionary, optional Tavily research and LLM, semantic vision provider, and YouTube OAuth. Save the OAuth client first, choose **Connect YouTube**, finish the browser flow, verify the exact channel title/ID, and explicitly confirm that destination.
3. Run diagnostics.

...
**Build:** 0.1.0-alpha.11 published hosted Windows full-dataset performance release

**Release:** [v0.1.0-alpha.11](https://github.com/kaywhy331/Video/releases/tag/v0.1.0-alpha.11) unsigned Windows prerelease
...
The repository is a production-hardened alpha with the local P0 buildout and bounded P1 operations/analytics foundations implemented and tested. The automated suite covers migrations through 024, including verified pre-upgrade backup, whole-batch rollback, fresh/upgrade schema parity, and an actionable newer-schema guard; checkpoint-safe deferred pause; task-aware graceful quit/drain; a DB-backed global final-render lease; workflow-locked manual/private uploads; remount-safe catalog progress/cancellation; staged catalog evidence workflows; worker-backed catalog import; atomic project licensing; immutable Guided-script provenance; managed interrupted-render cleanup; durable automatic project/job/render recovery; exact Final Review revision routing and per-scene audit; a complete keyboard-accessible nine-tab project workspace; a three-pane workflow-locked storyboard recovery editor; manifest-authorized active-final captions; actionable and audited exceptions; Autopilot operations health; IPC and media-protocol security; sourced research and provider policy; canonical geography; semantic footage verification; verified-footage scripts and narration; perceptual global matching; HDR/log fail-closed color handling; deterministic editing; crop-qualified 1080p/4K/portrait output profiles; licensed-music mixing; render/package QC; bounded repair; portable export/rebuild; cadence scheduling; five-checkpoint retention analytics; reversible learning; and resumable-upload protocol helpers. Repeatable 26,000-row receipts cover production catalog counts/search, main-process responsiveness, and exact-SHA hosted Windows production-Electron startup, interaction, and background-render behavior. Canonical traceability maps all 175 PRD IDs: 160 have report-bound local assertions, hosted Windows package evidence qualifies `SYS-005` and `SYS-006`, and 13 remain external qualification gates. It is not yet production-qualified because these remaining gates have not been performed:
...
- representative non-CI Electron renderer startup, real scrolling/interaction and memory behavior, and responsiveness while rendering in the background; the exact-SHA hosted Windows run is supporting evidence but not representative-hardware qualification.

`production_ready` therefore remains `false`. See [Implementation Coverage](docs/IMPLEMENTATION-COVERAGE.md), [Production Hardening](docs/PRODUCTION-HARDENING.md), the historical [alpha.7 Validation Report](VALIDATION_REPORT.md), and the current release's machine-readable [alpha.11 release-evidence index](docs/release-evidence/v0.1.0-alpha.11.json). Current validation status and acceptance receipts are generated artifacts: run clean release validation or download the exact-workflow-SHA CI artifact rather than treating a tracked root receipt as current evidence.

## Implemented

- Electron, React, and TypeScript desktop shell with sandboxed renderer
- SQLite/WAL/FTS5 data store with 23 atomic forward migrations, nested savepoints, composite catalog-search indexes, immutable Guided-input provenance, deferred lifecycle intent at active-job checkpoints, OAuth channel binding, provider endpoint/credential-origin bindings, active-final publication identity, device-local media-tool trust, and source/package migration-parity preflight
...
- optional Tavily Search/Extract research with real-URL enforcement, app-owned sources, strict cited claim extraction, freshness/conflict omission, scene claim IDs, and centralized endpoint admission
- one main-process endpoint policy for research, language, vision, and HTTP TTS calls, with explicit managed/custom-remote/loopback-local trust, DNS and redirect validation, pinned resolution, bounded transport, and credential-to-origin binding
- cached provider receipts, persisted trust/auth/quota health, and monthly plus project-snapshot budgets before paid calls
...
- private-first YouTube workflow with single-use state and S256 PKCE, memory-only pending credentials, explicit exact-channel/replacement confirmation, authoritative managed active-final validation, full render/hash/package/thumbnail/channel snapshots, exact resumable-session recovery, stale-private exception handling, publish-race private reset, automatic upload/processing continuation, timed captions, thumbnails, optional playlist, configurable synthetic-media disclosure, and a mandatory final publish/schedule/Keep private gate
...
npm run qualify:production-recovery:candidates -- --database="<videofactory.sqlite>" --kind=render
...
`npm run validate` runs the full pipeline as development-qualified evidence and may describe a dirty checkout. `npm run validate:release` runs the same checks but refuses anything except a clean exact HEAD/tree, rechecks source and inputs at completion, and is the mode used by GitHub Actions. Both emit separate runtime and claims input manifests plus status and acceptance receipts; these generated files are ignored by Git. Historical-evidence validation also resolves the recorded tag, commit, trees, ancestry, and single-change index through Git, then rejects release tags, commits, workflow runs, published-asset counts, signing state, external-gate counts, or `production_ready` statements in tracked claim documents that conflict with that immutable index. Windows artifacts are written to `release/` with canonical upload-safe names in the form `VideoFactory-Desktop-<version>-<arch>.<ext>`. GitHub Actions packages Windows only after the exact workflow SHA passes release validation, downloads only that SHA's evidence, launches the ZIP and installed NSIS application with isolated data, and writes an exact artifact manifest plus `SHA256SUMS.txt`. Manifest generation and verification reject missing, dirty, stale, development-qualified, tree/input-mismatched, or artifact-mismatched evidence. Branch builds record their ref without claiming a release tag; tag builds must exactly match the package version. This hosted-runner smoke is supporting package evidence, not code signing or a clean-machine installation qualification.

On a clean exact Windows x64 checkout and representative operator hardware, `npm run qualify:electron-performance -- --mode=qualification --device-class="<non-sensitive hardware class>"` exercises the production Electron renderer with the full 26,000-row fixture and a concurrent FFmpeg render. Startup evidence performs and records one explicit warm-up launch before measuring the next launch against the five-second target. A passing run writes ignored `validation/results/electron-performance.json` and `validation/external-qualification/index.json` artifacts. A subsequent clean `npm run validate:release` re-hashes those bytes, re-runs their machine criteria, binds them to the same commit/tree, and may qualify only `CAT-001`, `CAT-009`, and `PERF-001` through `PERF-003`. GitHub's Windows job runs the same full fixture in supporting mode and retains an exact-SHA receipt, but CI evidence cannot qualify representative hardware. Missing evidence stays pending; malformed, moved, tampered, CI, Linux, dirty-source, reduced-fixture, missing-warm-up, or manual evidence fails closed. Every other external gate remains independent, so this receipt alone cannot set `production_ready` to `true`.
...
The crash-and-resume gate now has a separate forced-process-tree harness. On one representative non-CI Windows x64 machine, it records six real packaged-app drills—paid provider work, licensed ingest, render assembly, live YouTube resumable-session reuse, live YouTube remote-commit reuse, and representative restore/rebuild. Aggregate the six ignored raw observations with `npm run qualify:production-recovery`; the privacy-reduced exact-source receipt may qualify only `E2E-004`, and missing field observations leave that gate pending. See [production recovery qualification](docs/PRODUCTION-RECOVERY-QUALIFICATION.md) for destructive-test precautions and commands.
...
The renderer has no Node.js access. IPC inputs are schema-validated, navigation is deny-by-default, only allowlisted Envato/YouTube HTTPS URLs may open, credentials use Electron `safeStorage`, logs redact common secret and OAuth callback forms, and uploads begin private. Provider credentials are injected only by the main process after endpoint trust, DNS/address, redirect, and credential-origin checks; endpoint changes cannot inherit an old credential silently. OAuth state, PKCE verifier, authorization code, and candidate tokens stay in the main process; tokens become active only after exact-channel confirmation. Publication side effects revalidate the active final bytes and immutable package/channel snapshot, so a stale upload cannot pass the human publication gate. Privileged OAuth, provider, publication, retry, and media-tool rejections expose stable codes and recovery guidance and write one versioned, bounded, redacted security-event shape without callback data, credentials, content, operator reasons, or device paths. Media-tool subprocesses receive an allowlisted environment, and custom FFmpeg/FFprobe executions are rechecked against device-local canonical-path and SHA-256 trust immediately before launch. Dependency audit, lockfile, SBOM, and response requirements are recorded in [Dependency Security](docs/DEPENDENCY-SECURITY.md).

...
export class MediaService {
  ...
  private segmentRows(fileId: string, durationMs: number, width: number, height: number): MediaSegment[] {
    ...
    return generateSlidingWindows(durationMs).map((window, index) => ({
      ...
      qualityScore: Math.max(0.5, 1 - index * 0.035),
      blackFrameRisk: 0,
      freezeRisk: 0,
  ...
  async ingestAcquisition(acquisitionId: string, detectedPath: string): Promise<AssetFile> {
    ...
    const segments = this.segmentRows(fileId, durationMs, video.width, video.height);
  ...
  private assignSegments(
    ...
    const eligible = segments.filter(segment => segment.eligible1080p);
    if (!eligible.length) {
      this.db.raw.prepare(`
      ...
      this.db.raw.prepare(`
        INSERT INTO exceptions(
          ...
          'Downloaded footage is below the effective 1080p requirement.',
  ...
  private updateProjectAfterAcquisition(projectId: string): void {
    ...
    `).run(
      rejected.count ? 'BLOCKED' : 'STORYBOARD_FINAL',
      rejected.count ? 0.5 : 0.55,
      new Date().toISOString(),
      projectId
    );
  }

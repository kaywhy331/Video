...
if ([string]::IsNullOrWhiteSpace($LogPath)) {
  $LogPath = Join-Path $root 'VideoFactory-Last-Build.log'
}
@('VideoFactory Desktop build log', ('Started: ' + (Get-Date).ToString('o')), ('Folder: ' + $root), '') |
...
try {
  Write-Host 'VideoFactory Desktop - validate and build portable Windows package' -ForegroundColor Cyan
  ...
  Invoke-LoggedCommand $npm.Source @('run', 'package:win:portable') 'Portable Windows packaging'

  Write-Host "`nBuild complete. Check the release folder." -ForegroundColor Green
  exit 0
} catch {

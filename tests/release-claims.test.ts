...
describe('machine-verifiable release claims', () => {
  it('[REL-005] rejects stale tag, commit, run, asset, signing, readiness, and gate claims', () => {
    ...
    expect(() => assertReleaseClaims(changedDocument(
      'README.md',
      'https://github.com/kaywhy331/Video/releases/tag/v0.1.0-alpha.11',
      'https://github.com/kaywhy331/Video/releases/tag/v0.1.0-alpha.12'
    ))).toThrow(/unknown release URL or tag/i);

    expect(() => assertReleaseClaims(changedDocument(
      'VALIDATION_REPORT.md',
      'for `v0.1.0-alpha.7` at the same merge commit',
      'for `v0.1.0-alpha.12` at the same merge commit'
  ...
  it('[REL-005] keeps claim-free documents valid when later release indexes disagree', () => {
    ...
    later.path = 'docs/release-evidence/v0.1.0-alpha.12.json';
    later.index.releaseSource.tag = 'v0.1.0-alpha.12';
    later.index.publication.tag = 'v0.1.0-alpha.12';
    later.index.publication.url = 'https://github.com/kaywhy331/Video/releases/tag/v0.1.0-alpha.12';
    ...
    expect(() => assertReleaseClaims(value)).toThrow(/do not project historical evidence index.*alpha\.12/i);

    value.documents['FUTURE-RELEASE.md'] =
      'See the [future release evidence](docs/release-evidence/v0.1.0-alpha.12.json).';

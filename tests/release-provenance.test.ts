...
function writeQualifiedProductionRecoveryEvidence(root: string): void {
  const identity = gitIdentity(root);
  const source = {
    commit: identity.commit,
    tree: identity.tree,
  ...
  const receipt = qualifyingProductionRecoveryEvidence({ source, appVersion: '9.8.7-alpha.1' });
  ...
  const admitted = writeProductionRecoveryQualificationIndex({
    root,
    source,
    now: new Date('2026-08-26T21:01:00.000Z')
  });
  const projection = {
    index: admitted.index,
    receipts: admitted.receipts.map(item => ({
  ...
  const qualifiedCase = {
    ...
    classification: 'external',
    result: 'passed_external_qualification',
    artifacts: [EXTERNAL_QUALIFICATION_INDEX_PATH, PRODUCTION_RECOVERY_RECEIPT_PATH],
    externalEvidence: {
      kind: 'production_recovery',
      index: admitted.index,
      receipt: admitted.qualifiedById['E2E-004']!.evidence
    }
  };
  const summary = {
    total: 2,
  ...
  const acceptancePath = resolve(root, 'VALIDATION_ACCEPTANCE_RECEIPT.json');
  const acceptance = JSON.parse(readFileSync(acceptancePath, 'utf8'));
  acceptance.cases = [acceptance.cases[0], qualifiedCase];
  acceptance.summary = summary;
  acceptance.externalQualificationEvidence = projection;
  writeFileSync(acceptancePath, JSON.stringify(acceptance));
  ...
  status.acceptance = { receipt: 'VALIDATION_ACCEPTANCE_RECEIPT.json', ...summary };
  status.externalQualificationEvidence = projection;
  status.externalQualification = [{
    id: qualifiedCase.id,
    status: 'qualified',
    artifacts: qualifiedCase.artifacts,
    evidence: qualifiedCase.externalEvidence
  }];
  status.production_ready = true;
  writeFileSync(statusPath, JSON.stringify(status));
}
...
describe('release artifact provenance', () => {
  ...
  it('packages and re-admits the exact production-recovery receipt required by E2E-004', () => {
    const root = fixtureRoot();
    writeValidationEvidence(root);
    writeQualifiedProductionRecoveryEvidence(root);
    expect(run(root).status).toBe(0);
    expect(run(root, ['--verify']).status).toBe(0);
    const manifest = JSON.parse(readFileSync(resolve(root, 'release', 'RELEASE_PROVENANCE.json'), 'utf8'));
    expect(manifest.validation.externalQualificationEvidence.receipts).toEqual([
      expect.objectContaining({ kind: 'production_recovery' })
    ]);
    expect(manifest.artifacts.map((item: { name: string }) => item.name)).toContain(
      'EXTERNAL_PRODUCTION_RECOVERY.json'
    );
  });

...
describe('Electron production performance evidence', () => {
  ...
  it('fails closed for count drift, stalled renderers, missing FFmpeg progress, and malformed evidence', () => {
    ...
    const missingWarmup = evidence();
    missingWarmup.measurements.startup.warmupCompleted = false as true;
    expect(() => assessElectronPerformanceEvidence(missingWarmup)).toThrow(/warm-up launch/i);
  });
});

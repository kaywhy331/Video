...
The renderer gate has a canonical target runner. From a clean exact Windows x64 commit on representative non-CI hardware, run `npm run qualify:electron-performance -- --mode=qualification --device-class="<non-sensitive hardware class>"`, then run `npm run validate:release` without changing HEAD or the tree. The target runner records one explicit warm-up launch, measures the next production-Electron launch, and writes an ignored receipt plus SHA-256 index; acceptance validation re-verifies the warm-up, measurements, target eligibility, canonical paths, byte sizes, hashes, and exact source before qualifying only `CAT-001`, `CAT-009`, and `PERF-001` through `PERF-003`. Hosted Windows CI exercises the same full 26K workload in supporting mode and retains an exact-SHA diagnostic artifact, but its CI receipt is ineligible for field qualification. Release provenance carries and independently re-admits qualifying attachments. An absent index leaves the gates pending, while an invalid or unsupported index blocks validation. This evidence does not waive any other external gate.

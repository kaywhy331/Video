...
| YouTube private-first publishing | Implemented, external validation pending | Persisted resumable session protocol, duplicate guard, automatic private upload/processing continuation, polling/attachment receipts, and an unambiguous final human publish/schedule/Keep private gate. Manual upload/package-sync IPC now enters through the same WorkflowService job identity and project lock; live OAuth/API rehearsal remains unrun |

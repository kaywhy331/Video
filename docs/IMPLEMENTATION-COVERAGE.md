...
Updated 2026-08-27 for fail-closed selected-provider health admission and persisted YouTube API health after the alpha.9 release-receipt rollup. The alpha.9 tag remains immutable; this later remediation does not alter its historical release evidence.
...
- [x] Gate every new manual or scheduled Autopilot project on hard health for the selected production providers and confirmed uploader; classify and persist YouTube auth/quota/runtime failures, clear stale health only after explicit channel reconnection or a successful API call, and keep transient failures plus Studio restrictions retryable.
...
| Cost/quota controls | Implemented + tested | Cached calls are exempt; call receipts, monthly and project-snapshot hard budgets, and persisted endpoint-trust/auth/quota preflight cover research, LLM, vision, and HTTP TTS adapters. YouTube quota exhaustion is separately classified, persisted, shown in setup/operations health, and blocks new autonomous work without being confused with an API publication restriction; live quota behavior remains an external qualification gate |
...
| Autopilot cadence scheduler | Implemented + tested | Durable due-state, queue/provider/storage gates, authoritative selected-provider/uploader hard-health admission shared by manual and scheduled creation, pause/resume creation controls, startup/timer/manual evaluation, recoverable blocked status, Full Autopilot creation, and bounded Guided inputs; real scheduled production cadence remains externally unqualified |
...
Those measured service and main-process criteria pass. A separate production-Electron target runner measures an explicit warm launch under five seconds, actual 26,000-row scrolling/interaction and memory behavior, and responsiveness during a concurrent background render on representative Windows x64 hardware. Filtered Library searches no longer wait for unrelated global coverage aggregation, and the isolated performance profile respects the hard first-run gate by measuring only its allowed Settings/Library transitions. Hosted Windows CI runs the same full fixture in supporting mode and retains its exact-SHA receipt on every workflow, but cannot claim representative non-CI qualification. No qualifying target receipt is stored in source, so the current historical release remains unqualified on those gates; a generated receipt can close them only after the acceptance pipeline re-verifies its exact-source evidence index.

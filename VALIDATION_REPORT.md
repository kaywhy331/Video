...
| Release artifact provenance | Passed in alpha.7 tag CI: canonical upload-safe naming, exact manifest/checksum verification, unsafe/extra/tampered artifact rejection, matching validation and package-smoke receipts, tag/version enforcement, truthful tag recording, and nine verified manifest artifacts |
...
The alpha.7 release chain passed PR run [32769307075](https://github.com/kaywhy331/Video/actions/runs/32769307075) for branch-head commit `16ff2b4` with its exact `${{ github.sha }}` Linux-to-Windows artifact handoff keyed to temporary merge SHA `b160047`, post-merge `main` run [32770075245](https://github.com/kaywhy331/Video/actions/runs/32770075245) for merge commit `b77535f`, and tag run [32770907678](https://github.com/kaywhy331/Video/actions/runs/32770907678) for `v0.1.0-alpha.7` at the same merge commit:

| Job | Result |
|---|---|
| Linux `validate` | Passed in 1 minute 5 seconds on the PR, 1 minute 23 seconds post-merge, and 1 minute 27 seconds on the tag |
| Windows `package-windows` | Passed in 6 minutes 5 seconds on the PR, 6 minutes 1 second post-merge, and 6 minutes 2 seconds on the tag |
| Packaged Windows lifecycle smoke | Passed in 46 seconds on the tag for ZIP launch and NSIS install/launch/uninstall |
| Release provenance | Tag/version/source matched; package-smoke evidence passed; nine release artifacts verified |
| Published prerelease | [v0.1.0-alpha.7](https://github.com/kaywhy331/Video/releases/tag/v0.1.0-alpha.7) contains 11 uploaded assets with GitHub SHA-256 digests matching the locally verified tag artifact |
...
The published tag remains unsigned and records `production_ready: false`; none of the 15 external qualification gates are waived by these receipts.
